...
const Outcome = () => {
  ...
  let outcomeFiltered = watch("outcome")
    ? watch("outcome").replace(",", ".")
    : null;
  let outcomeName = watch("name");

  const addOutcome = async () => {
    const docRef = await addDoc(collection(db, "outcome"), {
      outcomeName: outcomeName,
      outcomeValue: outcomeFiltered,
  ...
  const getOutcomes = async () => {
    ...
    outcomes.forEach((singleoutcome) => {
      let outcome = singleoutcome.outcomeValue;
      outcomesTable.push(Number(outcome));
    });
    let sum = 0;
    ...
    <BackgroundGradient marginHorizontal={0} justifyContent={"center"} flex={1}>
          ...
          <>
            ...
            {outcomes.length > 0 ? (
              <>
                ...
                >
                  {outcomes[outcomes.length - 1].date}
                </Text>
                <Text
                  style={{
                    fontFamily: "Open-Sans-Bold",
        ...
        <>
          {outcomes.length > 0 ? (
            <FlatList
              ...
              data={outcomes.sort((a, b) => a.sortDate - b.sortDate)}
              ...
              renderItem={({ item, index }) => (
                <LinearGradient
                  ...
                  style={{
                    ...
                    marginBottom: index === outcomes.length - 1 ? 20 : 5,
                    borderRadius: 5,
                  }}
                >
                  <View
                  ...
                  >
                    <TouchableOpacity
                      onPress={async () => {
                        await deleteDoc(doc(db, "outcome", item.id));
                        setActiveIndicator(true);
                        getOutcomes();
                      }}
                      style={{
      ...
      <Portal>
        ...
        >
          ...
          />
          <Button
            onPress={handleSubmit(confirmAddOutcome)}
            labelStyle={{
              fontFamily: "Open-Sans-Bold",
              fontSize: 14,
              flex: 1,
